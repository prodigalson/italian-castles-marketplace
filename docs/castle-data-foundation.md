...
The 2026-07-31 castle expansion uses a JamesEdition category-card snapshot as a manual/link-only source fixture. JamesEdition exposed 229 Italy castle results in the reviewed category pages, which is sufficient to meet the 100-listing threshold without using blocked or permission-gated sources. The refresh keeps only high-level card facts needed for discovery and filtering: location, bedrooms, bathrooms, living area, broad asset type, source attribution, source status, last-checked time, and a JamesEdition source page link. It does not copy listing photos, long descriptions, seller contact data, hidden map endpoints, AJAX/feed responses, or inquiry flows.
